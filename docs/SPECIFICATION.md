...
| 機能カテゴリ       | 内容                                                                        |
| ------------------ | --------------------------------------------------------------------------- |
| 予約管理（CRUD）   | 日付・時間帯・施設名・コート種類・定員・作成者・メッセージ登録/編集/削除、完了自動化  |
| 定員管理           | 参加表明時に定員チェック、定員到達で自動的にステータスを「締切」に変更      |
| 参加表明           | 参加/保留/削除 の選択、コメント（1件、改行可）、参加者一覧表示              |
...
* グループ全体の週単位・月単位練習回数および練習時間を集計し、**棒グラフで表示**。グラフの色分けはコート種類（不明も含む）ごとに行う。
* 個人をプルダウン選択すると該当メンバーの同指標が棒グラフで表示される（平均との比較は行わない）。
