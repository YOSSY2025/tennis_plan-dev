...
実績としてカウントされるための条件：
1. **ステータスが「完了」**(抽選中、募集中、中止はカウント対象外)
2. **参加者（participants）に含まれている**（保留「consider」はカウント対象外）
3. 上記2条件を満たす予約のみ、練習回数・練習時間に集計

### ● 集計・表示方法

* グループ全体の月単位練習回数および練習時間を集計し、表または**棒グラフで表示**。
* 個人別集計やグラフ表示により、選択した個人の上達具合（例: 参加頻度や練習時間の増減）を可視化し、練習との相関を分析。
* 期間指定により、任意の期間での実績集計が可能。
