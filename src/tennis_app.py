...
def entry_form_dialog(mode, idx=None, date_str=None):
    ...
    # --- B. 編集モード ---
        ...
        if facility_address:
            escaped_address = facility_address.replace("'", "\\'").replace('"', '&quot;')
            st.markdown(f'<div style="font-size: 11px; background-color: #f0f2f6; padding: 4px 8px; border-radius: 4px; margin-top: 4px; font-family: monospace;"><a href="#" onclick="navigator.clipboard.writeText(\'{escaped_address}\'); event.preventDefault(); return false;" style="color: #333; text-decoration: none;">{facility_address}</a></div>', unsafe_allow_html=True)
